...
エージェント・ネイティブ開発リポジトリ（multi-agent テンプレート）。
...
- 資格情報・秘密情報をコミットしない（`.hive/` `.cube/` は gitignore 済み）。
- 未信頼コードを devShell で直接実行しない（`scripts/sandbox_run.py` を用いる）。
- `hive admin *`（trust 昇格等の管理操作）はエージェントが実行しない（人間の管理操作）。
