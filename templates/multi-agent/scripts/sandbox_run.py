...
def _ensure_shim_running(cube_dir: Path, *, wait: float | None = None) -> None:
    ...
    起動プロセスが早期終了しても **deadline（``wait``）まで待ち続ける**: 即時終了は
    「並行起動の flock 敗者」（勝者は deadline 内の任意の時点で接続可能になる——
    マルチエージェントの同時初回実行）と「設定ミス等の単独失敗」を起動側から区別
    できないため。単独失敗の診断はタイムアウト時に、記録した終了コードとログ末尾を
    添えてまとめて示す。
    """
    if wait is None:
        try:
